...
export function StatCards({ visitors }: StatCardsProps) {
  ...
  const todaysVisitors = visitors.filter(v => new Date(v.checkInAt).toDateString() === today);
  ...
  const overstaying = currentlyInside.filter(v => {
    const hours = (Date.now() - new Date(v.checkInAt).getTime()) / (1000 * 60 * 60);
    return hours > 8;
  });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      {cards.map((card, i) => (
        <div key={i} className={`bg-white rounded-xl border p-4 flex items-center justify-between ${card.alert ? 'border-cs-red shadow-sm' : 'border-cs-gray-200'}`}>
          <div>
            <p className="text-sm font-medium text-cs-gray-500 mb-1">{card.title}</p>
            <h3 className="text-2xl font-bold font-heading text-cs-black flex items-center gap-2">
              {card.value}
              {card.alert && (
