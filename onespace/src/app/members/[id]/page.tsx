...
import { ArrowLeft, Sparkles, Calendar, Phone, Mail, MapPin, CreditCard, Activity, CheckCircle2, AlertTriangle, FileText, Plus } from "lucide-react";
...
export default function MemberProfilePage() {
  ...
  if (!member) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <button onClick={() => router.back()} className="mt-4 text-cs-red underline">Go back</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] animate-in fade-in duration-500 bg-cs-gray-50/30">
      ...
      {/* Left Column: Identity & Risk (Sticky) */}
      <div className="w-[340px] border-r border-cs-gray-200 bg-white flex flex-col h-full overflow-y-auto">
        <div className="p-6 pb-0">
          <button onClick={() => router.back()} className="flex items-center gap-2 text-sm text-cs-gray-500 hover:text-cs-black mb-6 transition-colors">
          ...
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium px-2.5 py-1 bg-cs-gray-100 text-cs-gray-700 rounded-md border border-cs-gray-200">{branchName}</span>
              <span className="text-xs font-medium px-2.5 py-1 bg-cs-gray-100 text-cs-gray-700 rounded-md border border-cs-gray-200 capitalize">{member.planType.replace("_", " ")}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold uppercase text-cs-gray-400 tracking-wider">Retention Risk</h3>
          </div>
          ...
          <ul className="space-y-3 mb-6">
            <li className="flex items-start gap-2 text-sm text-cs-gray-700">
              {riskScore >= 70 ? <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" /> : <CheckCircle2 className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />}
              <span>{member.daysSinceLastVisit === 0 ? "Active recently" : `Last visit ${member.daysSinceLastVisit} days ago (avg: ${member.avgVisitsPerMonth}/mo)`}</span>
            </li>
            <li className="flex items-start gap-2 text-sm text-cs-gray-700">
              {member.tickets.length > 0 ? <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" /> : <CheckCircle2 className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />}
              <span>{member.tickets.length} open support tickets</span>
            </li>
            <li className="flex items-start gap-2 text-sm text-cs-gray-700">
              <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" />
              <span>Contract ends on {new Date(member.contractEnd).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
            </li>
          </ul>
        ...
        {/* Quick Stats */}
        <div className="p-6">
          <h3 className="text-sm font-bold uppercase text-cs-gray-400 tracking-wider mb-4">Quick Stats</h3>
          <div className="space-y-4">
            <div className="flex justify-between items-center text-sm">
              <span className="text-cs-gray-500 flex items-center gap-2"><CreditCard className="w-4 h-4" /> MRR</span>
              <span className="font-semibold text-cs-black">{formatCurrency(member.monthlyFee)}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-cs-gray-500 flex items-center gap-2"><Calendar className="w-4 h-4" /> Member since</span>
              <span className="font-semibold text-cs-black">{member.memberSince}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-cs-gray-500 flex items-center gap-2"><MapPin className="w-4 h-4" /> Seat</span>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
            ...
            {[
              ...
              <button
                ...
                className={`pb-3 text-sm font-medium transition-colors relative ${
                  activeTab === tab.id ? "text-cs-red" : "text-cs-gray-500 hover:text-cs-black"
                }`}
              >
                {tab.label}
                {activeTab === tab.id && (
                  <div className="absolute bottom-[-2px] left-0 right-0 h-0.5 bg-cs-red" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8">
            ...
            <div className="max-w-3xl space-y-8 animate-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="bg-white rounded-xl border border-cs-gray-200 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button className="text-sm text-cs-red font-medium hover:underline">Edit</button>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-cs-black flex items-center gap-2">
                      <Mail className="w-4 h-4 text-cs-gray-400" /> {member.email}
                    </p>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-cs-gray-500 uppercase">Phone Number</label>
                    <p className="mt-1 text-sm text-cs-black flex items-center gap-2">
                      <Phone className="w-4 h-4 text-cs-gray-400" /> {member.phone}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="relative border-l-2 border-cs-gray-100 ml-3 space-y-8">
                  <div className="relative pl-6">
                    <div className="absolute left-[-9px] top-1 w-4 h-4 rounded-full bg-cs-red/20 border-2 border-white flex items-center justify-center">
                      <div className="w-2 h-2 rounded-full bg-cs-red"></div>
                    </div>
                    <p className="text-sm font-semibold text-cs-black">Checked In</p>
                    <p className="text-xs text-cs-gray-500 mt-0.5">{member.daysSinceLastVisit === 0 ? "Today, 9:14 AM" : `${member.daysSinceLastVisit} days ago`}</p>
                  </div>
                  <div className="relative pl-6">
                  ...
                  <div className="relative pl-6">
                    ...
                    <p className="text-xs text-cs-gray-500 mt-0.5">May 1, {formatCurrency(member.monthlyFee)}</p>
                  </div>
                </div>
              </div>
          ...
          {(activeTab === "bookings" || activeTab === "invoices") && (
            <div className="flex flex-col items-center justify-center py-20 text-center animate-in slide-in-from-bottom-2 duration-300">
              ...
              <h3 className="text-lg font-semibold text-cs-black mb-1">No data available</h3>
              <p className="text-sm text-cs-gray-500">This section is currently empty for this member.</p>
            </div>
          )}
        </div>
      </div>
