...
function zoneCategory(type: Zone["type"]): ZoneCategory | null {
  if (type === "hot_desk") return "hot_desk";
  if (type === "dedicated") return "dedicated";
  if (type === "cabin") return "cabin";
  if (type === "conference" || type === "phone_booth") return "meeting";
        ...
        >
          ...
          >
            ...
            {zones.length > 3 && (
              <line
                x1={20}
                y1={zones[0].y + zones[0].height + 10}
                x2={canvasWidth - 20}
                y2={zones[0].y + zones[0].height + 10}
                stroke="#CBD5E1"
                strokeWidth={0.6}
                strokeDasharray="6 3"
                opacity={0.5}
            ...
            {/* ──── SEATS ──── */}
            {seats.map((seat) => {
              const visible = zoneIdToVisible.get(seat.zoneId) ?? true;
