    ...
    case "cabin_6": {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        seats.push({
          ...
          zoneId: zId,
          code: `C-${existingSeatsCount + i + 1}`,
          x: zone.x + 15 + (col * (SEAT_SIZE + GAP)),
          y: zone.y + 25 + (row * (SEAT_SIZE + GAP + 10)),
          width: SEAT_SIZE,
          height: SEAT_SIZE,
          shape: "rect",
          status: "available",
    ...
    case "business_suite_13": {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        seats.push({
          ...
          x: zone.x + 10 + (col * (SEAT_SIZE + 8)),
          y: zone.y + 20 + (row * (SEAT_SIZE + 15)),
          width: SEAT_SIZE,
          height: SEAT_SIZE,
          shape: "rect",
          status: "available",
